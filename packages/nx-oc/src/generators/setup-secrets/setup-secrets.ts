...
export default async function (host: Tree, options: Schema) {
  ...
  if (serverUrl) {
    ...
    console.log(
      ok ? '✓ OPENSHIFT_SERVER set' : '✗ Failed to set OPENSHIFT_SERVER',
    );
  } else {
    console.log(
      '⚠  Could not determine OpenShift server URL — set OPENSHIFT_SERVER manually.',
    );
  }
  ...
  if (saToken) {
    ...
    console.log(
      ok ? '✓ OPENSHIFT_TOKEN set' : '✗ Failed to set OPENSHIFT_TOKEN',
    );
  } else {
    console.log(
      '⚠  Could not retrieve github-actions token — set OPENSHIFT_TOKEN manually.',
    );
  }
  ...
  if (!org) {
    console.log(
      '⚠  Could not derive GitHub org from remote — create ghcr-pull-secret manually.',
    );
    return;
  }
  ...
  const secretCreated = createDockerRegistrySecret(
    'ghcr-pull-secret',
    'ghcr.io',
    org,
    pat,
  ...
  if (secretCreated) {
    ...
    const linked = linkSecretToServiceAccount(
      'ghcr-pull-secret',
      'github-actions',
      options.infra,
    );
    console.log(
      linked
        ? '✓ ghcr-pull-secret linked to github-actions service account'
        : '✗ Failed to link secret',
    );
  } else {
