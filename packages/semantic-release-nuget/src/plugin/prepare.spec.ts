...
describe('prepare', () => {
  ...
  it('uses custom configuration when provided', async () => {
    await prepare(
      { project: 'MyProject.csproj', configuration: 'Debug' },
      makeContext(),
    );
  ...
  it('includes --no-build when noBuild is true', async () => {
    await prepare(
      { project: 'MyProject.csproj', noBuild: true },
      makeContext(),
    );
  ...
  it('omits --no-build when noBuild is false', async () => {
    await prepare(
      { project: 'MyProject.csproj', noBuild: false },
      makeContext(),
    );
  ...
  it('includes --include-symbols when set', async () => {
    await prepare(
      { project: 'MyProject.csproj', includeSymbols: true },
      makeContext(),
    );
  ...
  it('includes --include-source when set', async () => {
    await prepare(
      { project: 'MyProject.csproj', includeSource: true },
      makeContext(),
    );
  ...
  it('includes --serviceable when set', async () => {
    await prepare(
      { project: 'MyProject.csproj', serviceable: true },
      makeContext(),
    );
  ...
  it('packs each project when project is an array', async () => {
    ...
    expect((mockedExecFile as unknown as jest.Mock).mock.calls[0][1]).toContain(
      'Lib1.csproj',
    );
    expect((mockedExecFile as unknown as jest.Mock).mock.calls[1][1]).toContain(
      'Lib2.csproj',
    );
  });
