...
const LEGACY_GUIDANCE_SECTION_IDS = [
  'check-hook',
  'secret-scan',
  'dependency-guidance',
];
...
function buildAgentGuidance(options: NormalizedSchema): string {
  ...
  const groups = GUIDANCE_GROUPS.map((group) => {
    const items = group.items.map((item) =>
      readGuidanceFile(group.folder, `${item}.md`),
    );
    return `### ${group.title}\n\n${items.join('\n\n')}`;
  });
...
function applyDenyRulesStep(host: Tree): void {
  ...
  writeJson(host, CLAUDE_SETTINGS_PATH, {
    permissions: { deny: CLAUDE_DENY_PATTERNS },
  });
}
...
function applyAgentGuidance(host: Tree, options: NormalizedSchema): void {
  ...
  mergeManagedSection(
    host,
    AGENT_GUIDANCE_SECTION_ID,
    buildAgentGuidance(options),
  );
