...
This writes `.github/workflows/pull-request.yml` (the PR check), `.github/workflows/pipeline.yml` (build and promote on push to `main`), and `.openshift/environments.yml`. OpenShift deployment manifests are only auto-generated by the app generators when this pipeline setup is in place.
